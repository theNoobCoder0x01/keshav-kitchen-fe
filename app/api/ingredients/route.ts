...
export async function PUT(request: Request) {
  try {
    const { searchParams } = new URL(request.url);
    const id = searchParams.get('id');
    if (!id) return NextResponse.json({ error: 'Ingredient id required.' }, { status: 400 });
    const data = await request.json();
    const ingredient = await prisma.ingredient.update({ where: { id }, data });
    return NextResponse.json(ingredient);
  } catch (error) {
    return NextResponse.json({ error: 'Failed to update ingredient.' }, { status: 400 });
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to delete ingredient.' }, { status: 400 });
  }
}
