...
export async function GET(request: Request) {
  try {
    ...
    if (id) {
      ...
      if (!dailyMenu) return NextResponse.json({ error: 'Daily menu not found.' }, { status: 404 });
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to create daily menu.' }, { status: 400 });
  }
}
...
export async function PUT(request: Request) {
  try {
    const { searchParams } = new URL(request.url);
    const id = searchParams.get('id');
    if (!id) return NextResponse.json({ error: 'Daily menu id required.' }, { status: 400 });
    const data = await request.json();
    const dailyMenu = await prisma.dailyMenu.update({ where: { id }, data });
    return NextResponse.json(dailyMenu);
  } catch (error) {
    return NextResponse.json({ error: 'Failed to update daily menu.' }, { status: 400 });
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to delete daily menu.' }, { status: 400 });
  }
}
