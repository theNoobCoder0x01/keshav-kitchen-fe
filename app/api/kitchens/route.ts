...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to create kitchen.' }, { status: 400 });
  }
}
...
export async function PUT(request: Request) {
  try {
    const { searchParams } = new URL(request.url);
    const id = searchParams.get('id');
    if (!id) return NextResponse.json({ error: 'Kitchen id required.' }, { status: 400 });
    const data = await request.json();
    const kitchen = await prisma.kitchen.update({ where: { id }, data });
    return NextResponse.json(kitchen);
  } catch (error) {
    return NextResponse.json({ error: 'Failed to update kitchen.' }, { status: 400 });
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to delete kitchen.' }, { status: 400 });
  }
}
