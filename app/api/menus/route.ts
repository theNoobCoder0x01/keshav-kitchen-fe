...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to create menu.' }, { status: 400 });
  }
}
...
export async function PUT(request: Request) {
  try {
    const { searchParams } = new URL(request.url);
    const id = searchParams.get('id');
    if (!id) return NextResponse.json({ error: 'Menu id required.' }, { status: 400 });
    const data = await request.json();
    const menu = await prisma.menu.update({ where: { id }, data });
    return NextResponse.json(menu);
  } catch (error) {
    return NextResponse.json({ error: 'Failed to update menu.' }, { status: 400 });
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to delete menu.' }, { status: 400 });
  }
}
