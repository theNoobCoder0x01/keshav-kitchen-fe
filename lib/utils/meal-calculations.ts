 ...
 */
export function calculateMealMetrics(input: MealCalculationInput): MealCalculationResult {
  ...
  const ingredientCalculations: IngredientCalculation[] = ingredients.map(ingredient => {
    const weightInGrams = convertToGrams(ingredient.quantity, ingredient.unit);
  ...
  const totalWeightGrams = ingredientCalculations.reduce(
    (sum, ing) => sum + ing.weightInGrams, 0
  );
  ...
  // Person calculations
  const personsPerGhan = servingAmountInGrams > 0 
    ? ghanWeightGrams / servingAmountInGrams 
    : 0;
