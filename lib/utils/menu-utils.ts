 ...
 */
export function getSortedMenuGroupNames(
  groupedIngredients: GroupedMenuIngredients,
): string[] {
 ...
 */
export function hasCustomMenuGroups(
  ingredientGroups?: MenuIngredientGroup[],
): boolean {
 ...
 */
export function createUngroupedMenuGroup(
  ingredients: MenuIngredient[],
): GroupedMenuIngredients {
  return {
    Ungrouped: {
 ...
 */
export function calculateMenuGroupQuantity(
  ingredients: MenuIngredient[],
): number {
  return ingredients.reduce((sum, ingredient) => sum + ingredient.quantity, 0);
}
