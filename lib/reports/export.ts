...
export async function createReportPDF(
  ...
  doc.text("Name", startX + 8, tableTop + 6, {
    width: colWidths[0] - 16,
    continued: true,
  });
  doc.text("Weight", startX + colWidths[0] + 8, tableTop + 6, {
    width: colWidths[1] - 16,
    continued: true,
  });
  doc.text("Quantity", startX + colWidths[0] + colWidths[1] + 8, tableTop + 6, {
  ...
  data.forEach((item, idx) => {
    ...
    doc
      ...
      .fillAndStroke(isEven ? "#fcfcfc" : "#f0f0f0", "#e0e0e0");
    doc.fillColor("#111");
    doc.text((item as any).name || "", startX + 8, y + 6, {
      width: colWidths[0] - 16,
      continued: true,
    });
    doc.text((item as any).weight || "", startX + colWidths[0] + 8, y + 6, {
      width: colWidths[1] - 16,
      continued: true,
    });
    doc.text(
  ...
  doc
    .fontSize(11)
    .fillColor("#555")
    .text(`Total items: ${data.length}`, { align: "right" });
