...
export function createRecipeExcelWorksheet(recipe: RecipeExportData): XLSX.WorkSheet {
  const data: any[][] = [];
  ...
  data.push(['Created Date', recipe.createdAt ? new Date(recipe.createdAt).toLocaleDateString() : 'N/A']);
  data.push([]); // Empty row
  
  // Ingredients section
  data.push(['INGREDIENTS']);
  data.push(['Ingredient Name', 'Quantity', 'Unit', 'Cost per Unit', 'Total Cost']);
...
export function createRecipesExcelWorkbook(recipes: RecipeExportData[]): Buffer {
  ...
  summaryData.push(['Recipe Name', 'Category', 'Servings', 'Ingredients Count', 'Total Cost']);
  ...
  if (recipes.length > 10) {
    summaryData.splice(3, 0, ['Note: Only first 10 recipes shown as individual sheets due to Excel limitations']);
  }
...
export function createRecipeCSV(recipe: RecipeExportData): string {
  ...
  lines.push(`Created Date,"${recipe.createdAt ? new Date(recipe.createdAt).toLocaleDateString() : 'N/A'}"`);
...
export function createRecipesCSV(recipes: RecipeExportData[]): string {
  ...
  lines.push('Recipe Name,Category,Subcategory,Servings,Ingredients Count,Total Cost,Cost per Serving');
  ...
  recipes.forEach((recipe, index) => {
    ...
    lines.push(`RECIPE ${index + 1}: ${encodeTextForPDF(recipe.name).toUpperCase()}`);
    lines.push(''); // Empty line
