import {
  addGujaratiFontSupport,
  containsGujaratiText,
  encodeTextForPDF,
...
export function createMenuReportPDFWithJsPDF(
  ...
  if (type === "ingredients") {
    ...
    if (data.summary) {
      ...
      addUnicodeText(
        doc,
        `Unique Ingredients: ${data.summary.uniqueIngredients}`,
        20,
        yPosition,
      );
      yPosition += 6;
      ...
      if (data.selectedMealTypes && data.selectedMealTypes.length > 0) {
        addUnicodeText(
          doc,
          `Meal Types: ${data.selectedMealTypes.join(", ")}`,
          20,
          yPosition,
        );
        yPosition += 6;
    ...
    if (data.combinedIngredients && data.combinedIngredients.length > 0) {
      const tableData = data.combinedIngredients.map((ingredient) => [
        ...
        ingredient.sources
          .map((s) => `${s.kitchen} - ${s.mealType}`)
          .join(", "),
      ]);

      // Prepare data with Unicode support
      const unicodeTableData = prepareTableData(tableData);
      const unicodeHeaders = prepareTableData([
        ["Ingredient", "Total Qty", "Unit", "Cost", "Sources", "Details"],
      ]);
      ...
      addUnicodeText(
        doc,
        "No ingredients found for the selected criteria",
        20,
        yPosition,
      );
    }
    ...
    addUnicodeText(
      doc,
      `Breakfast Count: ${data.breakfastCount || 0}`,
      20,
      yPosition,
    );
    yPosition += 6;
    ...
    addUnicodeText(
      doc,
      `Dinner Count: ${data.dinnerCount || 0}`,
      20,
      yPosition,
    );
    yPosition += 6;
    ...
    if (data.selectedMealTypes && data.selectedMealTypes.length > 0) {
      addUnicodeText(
        doc,
        `Selected Meal Types: ${data.selectedMealTypes.join(", ")}`,
        20,
        yPosition,
      );
      yPosition += 6;
    ...
    if (data.combinedIngredients && data.combinedIngredients.length > 0) {
      ...
      const ingredientsTableData = data.combinedIngredients.map(
        (ingredient) => [
          ingredient.name,
          ingredient.totalQuantity.toFixed(2),
          ingredient.unit,
      ...
      const unicodeIngredientsHeaders = prepareTableData([
    ...
    if (data.menus && data.menus.length > 0) {
      ...
      const unicodeMenuHeaders = prepareTableData([
    ...
    const unicodeSummaryHeaders = prepareTableData([
      ["Kitchen", "Recipe", "Meal Type", "Servings", "Status", "Notes"],
    ]);
    ...
    const unicodeMealHeaders = prepareTableData([
      ["Kitchen", "Recipe", "Servings", "Ghan Factor", "Status"],
    ]);
