...
function preparePrismaExcel(inputPath, outputPath) {
  ...
  const recipeIndexSheet = xlsx.utils.sheet_to_json(wb.Sheets[sheetNames[0]], {
    header: 1,
  });
  // Second column (index 1) contains recipe names (skip first row)
  const recipesList = recipeIndexSheet
    .slice(1)
    .map((r) => r[1])
    .filter(Boolean);
  ...
  xlsx.utils.book_append_sheet(
    newWb,
    xlsx.utils.aoa_to_sheet(recipesSheetData),
    "recipes",
  );

  recipesList.forEach((recipeName, idx) => {
    if (!sheetNames.includes(recipeName)) return;
    const rawSheet = xlsx.utils.sheet_to_json(wb.Sheets[recipeName], {
      header: 1,
    });

    // Find header row with "ક્રમ"
    const headerRowIdx = rawSheet.findIndex((row) =>
      row.some((cell) => String(cell || "").includes("ક્રમ")),
    );
    ...
    xlsx.utils.book_append_sheet(
      newWb,
      xlsx.utils.aoa_to_sheet(ingSheetData),
      recipeIds[idx],
    );
  });
...
// Run the script
preparePrismaExcel(
  "/Users/thenoob0x01/Downloads/liquid.xlsx",
  "/Users/thenoob0x01/Downloads/liquid_prisma_ready.xlsx",
);
