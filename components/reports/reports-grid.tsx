...
export function ReportsGrid({ onDownload }: ReportsGridProps) {
  ...
  const handleDeleteReport = async () => {
    if (
      !deleteConfirm.report ||
      !deleteConfirm.report.type ||
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      <ReportCard
        ...
        onEdit={(item) =>
          setEditDialog({ open: true, report: { ...item, type: "breakfast" } })
        }
        onDelete={(item) =>
          setDeleteConfirm({
            open: true,
            report: { ...item, type: "breakfast" },
          })
        }
      />
      <ReportCard
        ...
        onEdit={(item) =>
          setEditDialog({ open: true, report: { ...item, type: "lunch" } })
        }
        onDelete={(item) =>
          setDeleteConfirm({ open: true, report: { ...item, type: "lunch" } })
        }
      />
      <ReportCard
        title="Dinner"
        ...
        onEdit={(item) =>
          setEditDialog({ open: true, report: { ...item, type: "dinner" } })
        }
        onDelete={(item) =>
          setDeleteConfirm({ open: true, report: { ...item, type: "dinner" } })
        }
      />
      <AddEditReportDialog
        open={addDialog.open}
      ...
      <AddEditReportDialog
        ...
        initialReport={
          editDialog.report
            ? { ...editDialog.report, type: editDialog.report.type || "" }
            : undefined
        }
        onSave={(report) => {
          if (editDialog.report && editDialog.report.type) {
            handleEditReport({
              ...report,
              type: editDialog.report.type,
              id: editDialog.report.id,
            });
          }
        }}
      />
      <AlertDialog
        ...
        onOpenChange={() => setDeleteConfirm({ open: false, report: null })}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
